...
catches every `SignetError` and raises a generic DRF exception instead,
so the response does not disclose *which* check failed. A failed CSRF
check raises `PermissionDenied` (403); a bad signature, an expired token,
a revoked session and every other cause raise `AuthenticationFailed`
(401, when a Signet class is the view's first authenticator). Both carry
the same body. The CSRF check runs only after the access token verifies,
so the 403 tells nothing to anyone but the token's holder. The distinct types reach only the
...
|---|---|---|---|
| refresh | 403 | 401 | 401 |
| logout | 403 | 200 for a cookie client, 401 for a header one | 200 for a cookie credential, 401 for a header one |
...
Logout answers 200 to a cookie client with no credential, or one it
cannot revoke, because the browser ends up signed out either way; a
header client has no cookies to clear, so it is not told it signed out. See
{doc}`views`.
